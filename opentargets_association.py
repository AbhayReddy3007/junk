...
def _fallback_search_terms(ind: str) -> list[str]:
    """
    Generate progressively simpler search terms for an indication
    when the primary disease ID yields no scores.
    Tries:
      1. Strip content in parentheses  e.g. "Alcohol Use Disorder (AUD)" -> "Alcohol Use Disorder"
      2. First two words               e.g. "Type 2 Diabetes Mellitus (Prevention)" -> "Type 2 Diabetes"
      3. First word                    e.g. "Hypertension"
    ...
    # First 3 words
    words = stripped.split() if stripped else ind.split()
    if len(words) > 3:
