...
def _ot_post(query: str, variables: dict, context: str = "") -> dict | None:
    """
    Shared helper: POST to OT GraphQL, raise on HTTP error,
    surface GraphQL-level errors, and return the data dict.
    """
    try:
        r = requests.post(
            OT_GRAPHQL,
        ...
        body = r.json()
        if "errors" in body:
            log.warning("OT GraphQL error%s: %s", f" [{context}]" if context else "", body["errors"])
            return None
        return body.get("data")
    except Exception as exc:
        log.warning("OT request failed%s: %s", f" [{context}]" if context else "", exc)
        return None
...
def ot_search_target(name: str) -> tuple[str | None, str | None]:
    ...
    """
    data = _ot_post(query, {"q": name}, context=f"target search: {name}")
    if data:
...
def ot_search_disease(name: str) -> tuple[str | None, str | None]:
    ...
    """
    data = _ot_post(query, {"q": name}, context=f"disease search: {name}")
    if data:
        hits = data.get("search", {}).get("hits", [])
        if hits:
            h = hits[0]
...
def ot_association_score(disease_id: str, target_id: str) -> float | None:
    ...
    Uses target -> associatedDiseases with a disease filter, which is the
    correct OpenTargets v4 GraphQL pattern.
    """
    query = """
    ...
    data = _ot_post(
        ...
        context=f"assoc score target={target_id} disease={disease_id}",
    )
    if data:
