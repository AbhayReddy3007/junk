...
def _score_indication(
    ...
    """
    Score one indication against all MoA targets in a SINGLE API call.

    Fetches the top 500 associated targets for the disease, then looks up
    each of our MoA targets by Ensembl ID client-side.
    This avoids N separate requests per target.
    """
    ...
    query = """
    query AssocBulk($diseaseId: String!) {
      disease(id: $diseaseId) {
        associatedTargets(
          enableIndirect: true
          page: { index: 0, size: 500 }
          orderByScore: true
        ) {
          rows {
            score
    ...
    """
    data = _ot_post(query, {"diseaseId": disease_id}, context=f"bulk-score:{disease_id}")
    ...
    if data:
        ...
        found = {r["target"]["id"]: r["score"] for r in rows}
        ...
        for tid, (moa, sym) in wanted.items():
            log.info("    %s (%s) × '%s' → N/A (fetch failed)", sym or moa, tid, ind)
