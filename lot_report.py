...
───────────────────────
Reads scored data for one molecule from BigQuery `LOT_TABLE` (populated by
``line_of_treatment.py`` / ``lot_scoring.py``) and generates one professional
PDF report using Gemini for narrative generation.

For each drug, the LATEST row per (drug_name, country) — by created_at — is
    used, together with the most recently computed final_lot_score for that
    drug. The table is append-only, so older scoring runs are discarded.
...
Each report is generated in memory as PDF bytes. The caller is responsible for
uploading those bytes through the shared dimension report upload helper.
...
def load_from_bigquery(molecule_name: str) -> list[dict]:
    """
    Load the LATEST row per (drug_name, country) from LOT_TABLE.
    Only the requested molecule is fetched.
    """
    bq_client = get_bq_client()
    table_ref = f"`{PROJECT_ID}.{BQ_DATASET_ID}.{LOT_TABLE}`"

    query_params = [bigquery.ScalarQueryParameter("molecule_name", "STRING", molecule_name)]

    query = f"""
        WITH ranked AS (
            SELECT
                drug_name, country, lot_score, lot_type, rationale,
                final_lot_score, created_at,
                ROW_NUMBER() OVER (
                    PARTITION BY drug_name, country ORDER BY created_at DESC
                ) AS _rn
            FROM {table_ref}
            WHERE drug_name = @molecule_name
        )
    ...
    logger.info("[LOT_REPORT] Loaded %d row(s) for molecule '%s'.", len(rows), molecule_name)
    return rows
...
def build_single_drug_report(stats: dict, narrative: dict) -> bytes:
    """Build a business-focused PDF report and return its bytes."""
    styles = build_styles()
    story = []
    buffer = BytesIO()
...
def generate_lot_report(molecule_name: str) -> bytes:
    """Generate one in-memory PDF report for one molecule.

    Args:
        molecule_name: Exact molecule name to load from LOT_TABLE.

    Returns:
        Generated PDF bytes.

    Raises:
        ValueError: If the molecule name is empty or no LOT data exists.
    ...
    if not API_KEY:
        raise RuntimeError("GEMINI_API_KEY not set — cannot generate LOT report.")

    rows = load_from_bigquery(molecule_name)
    if not rows:
        raise ValueError(f"No LOT data found for molecule: {molecule_name}")

    bucket = {
        "countries": sorted(rows, key=lambda row: row.get("country") or ""),
    ...
    bucket["final_lot_score"] = latest_row.get("final_lot_score")

    logger.info("[LOT_REPORT] Generating report for '%s'...", molecule_name)
    stats = extract_drug_stats(molecule_name, bucket)
    if not _is_data_sufficient(stats):
        logger.warning(
            "[LOT_REPORT] Insufficient LOT data for '%s' — generating report from what's available.",
            molecule_name,
        )

    narrative = generate_lot_narrative(stats)
    report_bytes = build_single_drug_report(stats, narrative)
    logger.info("[LOT_REPORT] Report generated for '%s'.", molecule_name)
    return report_bytes
