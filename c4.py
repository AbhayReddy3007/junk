...
def process():
    ...
    if missing_cols:
        ...
        if "phase" not in df.columns:
            ...
            # Only consider known phases (rank > -1) when computing the max per TA-I.
            # Mask just the rank column (not the whole row) so groupby keys stay intact.
            known_ranks = df["_phase_rank"].where(df["_phase_rank"] > -1)
            df["_max_known_rank"] = known_ranks.groupby(df["TA - I"]).transform("max")
