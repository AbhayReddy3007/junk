...
3. Deduplicates on TA - I by keeping only the highest-phase row(s).
   NOTE: Phase deduplication is applied to Clinical Trials rows only.
         Rows where data_source != "Clinical Trials" are never phase-deduped.
...
def process():
    ...
    if missing_cols:
        ...
        # We split them out now, null their fetched fields, and skip Steps 3-6.
        ...
        if "data_source" in df.columns:
            ds = df["data_source"].astype(str).str.strip().str.lower()
            ct_tai  = set(df.loc[ds == "clinical trials", "TA - I"].dropna().unique())
            all_tai = set(df["TA - I"].dropna().unique())
            # Only non-CT if the TA-I has zero Clinical Trials rows
            non_ct_tai_live = all_tai - ct_tai
        else:
            non_ct_tai_live = set()

        non_ct_tai = non_ct_tai_live | cached_non_ct_tai

        # Persist any newly discovered non-CT TA-Is to cache
        if non_ct_tai != cached_non_ct_tai and cache_path:
            cache[_NON_CT_CACHE_KEY] = sorted(non_ct_tai)
            _save_cache(cache_path, cache)
            new_entries = non_ct_tai - cached_non_ct_tai
            if new_entries:
                print(f"  Cached {len(new_entries)} new non-Clinical-Trials TA-I(s).")

        non_ct_mask = df["TA - I"].isin(non_ct_tai)
        ...
        if "phase" not in df.columns:
            ...
            # Identify which rows are Clinical Trials at the row level.
            if "data_source" in df.columns:
                ds_lower = df["data_source"].astype(str).str.strip().str.lower()
                is_ct_row = ds_lower == "clinical trials"
            else:
                is_ct_row = pd.Series(True, index=df.index)

            # Compute the max known phase per TA-I considering CT rows only.
            # Non-CT rows are excluded from the max calculation and always kept.
            known_ranks_ct = df["_phase_rank"].where(
                is_ct_row & (df["_phase_rank"] > -1)
            )
            df["_max_known_rank"] = known_ranks_ct.groupby(df["TA - I"]).transform("max")

            before = len(df)
            pre_step3 = df.copy()
            # Keep a row if:
            #   - it is NOT a Clinical Trials row (non-CT rows are never phase-deduped), OR
            #   - its phase rank is unknown (rank == -1), OR
            #   - its phase equals the highest known CT phase in the TA-I
            keep_mask = (
                ~is_ct_row |
                (df["_phase_rank"] == -1) |
