...
================================================
Reads an Excel file with 'indication', 'moa', and 'indication_type' columns,
filters to rows where indication_type is 'secondary',
maps MoA entries and indications to OpenTargets equivalents,
fetches association scores from OpenTargets GraphQL API,
and writes the highest association score per secondary indication back to the Excel file.
...
- Target-anchored disease resolution: fetches ALL diseases associated with the
  resolved MoA targets from OT and matches indications against that list first,
  before falling back to open text search or Gemini.
...
def _ot_post(query: str, variables: dict, context: str = "", max_retries: int = 3) -> dict | None:
    """Shared POST to OT GraphQL with retry + backoff on transient errors."""
    for attempt in range(1, max_retries + 1):
        try:
            r = requests.post(
                OT_GRAPHQL,
            ...
            if r.status_code in (429, 500, 502, 503, 504) and attempt < max_retries:
                wait = 2 ** attempt
                log.warning("OT HTTP %s [%s] — retrying in %ds (attempt %d/%d)",
            ...
            if attempt < max_retries:
                log.warning("OT timeout [%s] — retrying (attempt %d/%d)", context, attempt, max_retries)
                time.sleep(2 ** attempt)
                continue
            log.warning("OT timeout [%s] — giving up after %d attempts", context, max_retries)
            return None
        except Exception as exc:
...
def ot_search_disease(name: str) -> tuple[str | None, str | None]:
    ...
    Strategy:
      1. Search with the original query term.
      2. If the query is a single word, also search with quoted form and
         British/American spelling variants (e.g. dyslipidemia ↔ dyslipidaemia).
      3. Pool all hits and pick the best one using relevance scoring:
         - Prefer actual diseases over measurements/processes.
         - Prefer exact name matches or close name matches.
         - Penalize hits with many extra words (subtypes).
    """
    graphql = """
    ...
    # ── Gather candidates from multiple search attempts ───────────────────
    all_candidates: list[tuple[str, str]] = []   # (id, name)
    ...
    # Quoted exact search — useful for generic terms that OT splits
    if len(name.split()) == 1 and not name.startswith('"'):
        search_queries.append(f'"{name}"')

    for sq in search_queries:
        data = _ot_post(graphql, {"q": sq}, context=f"disease:{sq}")
        if data:
    ...
    for rank, (cid, cname) in enumerate(all_candidates):
        ...
        if cname_lower == query_lower:
            score += 50
        # Also check spelling variant match
        elif re.sub(r"aemia\b", "emia", cname_lower) == query_lower:
            score += 50
        elif re.sub(r"emia\b", "aemia", cname_lower) == query_lower:
            score += 50
        ...
        # Specificity penalty — penalize hits with many extra words (subtypes)
        extra_words = len(hit_words - query_words)
        score -= extra_words * 2

        # Search-rank tiebreaker
        score -= rank * 0.1
    ...
    if len(all_candidates) > 1:
        log.info("  OT disease search '%s' → picked %s (%s); other hits: %s",
                 ...
                 [(c, n) for c, n in all_candidates if c != best_id])
    else:
...
def fetch_all_target_diseases(
    ...
    """
    Fetch ALL diseases associated with the given Ensembl target IDs from OT.

    Uses associatedDiseases with pagination to retrieve the full disease list
    for each target, then deduplicates across targets.

    Returns a list of (disease_id, disease_name) tuples — the universe of
    diseases OT knows about for these targets. This becomes the candidate pool
    against which indications are matched in resolve_indications().
    """
    query = """
    ...
    for tid in target_ids:
        ...
        while True:
            data = _ot_post(
                ...
                context=f"target-diseases:{tid}:page{page_index}",
            )
            if not data:
                log.warning("  fetch_all_target_diseases: no data for %s page %d", tid, page_index)
                break

            assoc = data.get("target", {}).get("associatedDiseases", {})
            if total is None:
                total = assoc.get("count", 0)
                log.info("  Target %s has %d associated diseases in OT", tid, total)
    ...
    log.info(
        "fetch_all_target_diseases: %d unique diseases across %d targets",
...
def resolve_indications(
    ...
    cache: dict,
    ot_diseases: list[tuple[str, str]] | None = None,
) -> dict[str, tuple[str | None, str | None]]:
    """
...
def main():
    ...
    missing   = [c for c in ("indication", "moa", "indication_type") if c not in col_lower]
    if missing:
        sys.exit(f"❌  Missing columns: {missing}. Found: {list(df.columns)}")

    ind_col  = col_lower["indication"]
    moa_col  = col_lower["moa"]
    type_col = col_lower["indication_type"]

    raw_moa_str = df[moa_col].dropna().iloc[0]
    raw_moas    = [m.strip() for m in raw_moa_str.split(";") if m.strip()]

    # ── Filter to secondary indications only ──────────────────────────────────
    is_secondary = df[type_col].str.strip().str.lower() == "secondary"
    df_secondary = df[is_secondary]
    n_total = len(df)
    n_secondary = len(df_secondary)
    log.info("🔍 Filtered to %d / %d rows with indication_type='secondary'", n_secondary, n_total)
    if n_secondary == 0:
        sys.exit("❌  No rows with indication_type='secondary' found.")

    # ── Normalize indications (secondary only) ────────────────────────────────
    ...
    raw_inds_all  = df_secondary[ind_col].dropna().unique().tolist()
    ...
    # ── Step 1b: Fetch ALL diseases for the resolved targets from OT ──────────
    # Results are cached under "target_diseases" so subsequent runs skip the
    # paginated fetch unless --clear-cache or --force-resolve is passed.
    resolved_target_ids = sorted(tid for tid, _ in target_map.values() if tid)
    ot_diseases: list[tuple[str, str]] = []

    if resolved_target_ids:
        cache_key = "target_diseases"
        cached_td = cache.get(cache_key, {})
        # Cache key = sorted comma-joined Ensembl IDs of the resolved targets
        targets_fingerprint = ",".join(resolved_target_ids)
        cached_entry = cached_td.get(targets_fingerprint)

        if cached_entry and not FORCE_RESOLVE:
            ot_diseases = [tuple(pair) for pair in cached_entry]
            log.info(
                "\n── Step 1b: Loaded %d OT diseases from cache (targets: %s) ──",
                len(ot_diseases), targets_fingerprint,
            )
        else:
            log.info(
                "\n── Step 1b: Fetching all OT diseases for %d resolved target(s) ──",
                len(resolved_target_ids),
            )
            ot_diseases = fetch_all_target_diseases(resolved_target_ids)
            log.info(
                "   Retrieved %d unique diseases from OT target disease lists",
                len(ot_diseases),
            )
            # Persist to cache
            cached_td[targets_fingerprint] = ot_diseases
            cache[cache_key] = cached_td
            save_cache(cache)
    else:
        log.warning("  No targets resolved — skipping OT disease list fetch")

    # ── Step 2: Indications → diseases ────────────────────────────────────────
    # Tries: (1) cache → (2) OT target-disease list → (3) OT text search
    log.info("\n── Step 2: Resolving indications ────────────────────────────────")
    ind_map = resolve_indications(unique_inds, cache, ot_diseases=ot_diseases)

    # ── Step 3: Association scores ─────────────────────────────────────────────
    log.info("\n── Step 3: Fetching association scores ───────────────────────────")
    max_scores = compute_max_scores(unique_inds, ind_map, target_map)

    # ── Map results back to every raw *secondary* indication variant ─────────
    # Non-secondary rows get None (no score attempted).
    secondary_norms = set(raw_to_norm.values())  # only secondary indications were normalized

    def _lookup_score(raw_ind):
        nk = raw_to_norm.get(raw_ind)
        if nk is None:
            return None  # not a secondary indication
        rep = norm_to_raw.get(nk, raw_ind)
    ...
    log.info("  Secondary rows          : %d", n_secondary)
    log.info("  MoA targets resolved    : %d / %d", n_targets, len(raw_moas))
    log.info("  OT diseases fetched     : %d (from target disease lists)", len(ot_diseases))
    log.info("  Indications resolved    : %d / %d (secondary only)", n_ind, len(unique_inds))
